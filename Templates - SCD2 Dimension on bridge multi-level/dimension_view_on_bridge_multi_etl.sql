DROP VIEW IF EXISTS "INFORMATION_MARTS"."DIM_SHOP_LOCATION_EMPLOYEERANGE";
CREATE  VIEW "INFORMATION_MARTS"."DIM_SHOP_LOCATION_EMPLOYEERANGE"  AS 
	...
	( 
		SELECT 
			...
			, "SAT_SRC3_2_2"."HAS_PARKING" AS "HAS_PARKING"
			, "SAT_SRC3_2_2"."SHOP_SIZE" AS "SHOP_SIZE"
	...
	( 
		SELECT 
			  "DVO_BASE2"."BASE_OBJECT_H_KEY" AS "BASE_OBJECT_H_KEY"
			, "DVO_BASE2"."SNAPSHOT__TIMESTAMP" AS "SNAPSHOT__TIMESTAMP"
		FROM "DVO_BASE2" "DVO_BASE2"
		UNION 
		SELECT 
		...
		UNION 
		SELECT 
			  "DVO_BASE4"."BASE_OBJECT_H_KEY" AS "BASE_OBJECT_H_KEY"
			, "DVO_BASE4"."SNAPSHOT__TIMESTAMP" AS "SNAPSHOT__TIMESTAMP"
		FROM "DVO_BASE4" "DVO_BASE4"
	)
	, "SNAPSHOT_DATES" AS 
	( 
		SELECT 
	...
	( 
		SELECT 
			...
			, "DVO_BASE3"."HAS_PARKING" AS "HAS_PARKING"
			, "DVO_BASE3"."SHOP_SIZE" AS "SHOP_SIZE"
			, UPPER(SHA1_HEX(  UPPER(REPLACE(COALESCE(TRIM( "DVO_BASE3"."BASE_OBJECT_H_KEY"),'~'),'\#','\\' || '\#'))|| 
				'\#'           || UPPER(REPLACE(COALESCE(TRIM( TO_CHAR("DVO_BASE3"."HAS_PARKING")),'~'),'\#','\\' || '\#'))|| '\#' ||  UPPER(REPLACE(COALESCE(TRIM( "DVO_BASE3"."SHOP_SIZE"),'~'),'\#','\\' || '\#'))|| '\#'      )) AS "DIM_HASH_DIFF"
	...
	( 
		SELECT 
			...
			, "DATA_SET"."HAS_PARKING" AS "HAS_PARKING"
			, "DATA_SET"."SHOP_SIZE" AS "SHOP_SIZE"
	...
	( 
		SELECT 
			...
			, "HASH_DIFF_DATA_SET"."HAS_PARKING" AS "HAS_PARKING"
			, "HASH_DIFF_DATA_SET"."SHOP_SIZE" AS "SHOP_SIZE"
	...
	SELECT 
		...
		, COALESCE(LEAD("FILTERED_DATA_SET"."SNAPSHOT__TIMESTAMP")OVER(PARTITION BY "FILTERED_DATA_SET"."SHOPS_HKEY" ORDER BY "FILTERED_DATA_SET"."SNAPSHOT__TIMESTAMP")
			, TO_TIMESTAMP('31/12/2999 23:59:59' , 'DD/MM/YYYY HH24:MI:SS')) AS "END_SNAPSHOT__TIMESTAMP"
		, "FILTERED_DATA_SET"."HAS_PARKING" AS "HAS_PARKING"
		, "FILTERED_DATA_SET"."SHOP_SIZE" AS "SHOP_SIZE"
